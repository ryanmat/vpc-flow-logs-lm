#!/usr/bin/env bash
# Description: Imports the Azure VNet IP Usage DataSource JSON into LogicMonitor via REST API.
# Description: Uses Bearer token auth. Idempotent -- skips import if DataSource already exists.
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "  Examples:"
    echo "    $0 datasources/Azure_VNet_IPUsage.json"
    exit 1
fi
