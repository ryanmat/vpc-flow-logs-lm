...
VPC_FUNCTION="KPMGWebhookForwarderVPC"
VPC_STACK="kpmg-webhook-forwarder-vpc"
VPC_LOG_GROUP="/aws/vpc/flowlogs"
VPC_CONCURRENCY=5
VPC_STATEMENT_ID="AllowCWLogsVPC"

WAF_FUNCTION="KPMGWebhookForwarderWAF"
WAF_STACK="kpmg-webhook-forwarder-waf"
WAF_LOG_GROUP="aws-waf-logs-kpmg"
WAF_CONCURRENCY=1
WAF_STATEMENT_ID="AllowCWLogsWAF"

FILTER_NAME="KPMGWebhookForwarder"
...
wire_subscription() {
    ...
    aws logs delete-subscription-filter \
        --log-group-name "$LOG_GROUP" \
        --filter-name "KPMGWebhookForwarder" \
        --region "$REGION" 2>/dev/null || true
...
# Main deployment
# ============================================================
echo "=== Deploying KPMG Webhook Forwarders ==="
echo "Region:    $REGION"
...
OLD_STACK="kpmg-webhook-forwarder"
