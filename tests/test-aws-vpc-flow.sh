...
# Test 4: KPMGLMLogsForwarder Lambda exists
check "KPMGLMLogsForwarder Lambda function exists" \
    aws lambda get-function --function-name KPMGLMLogsForwarder $PROFILE_FLAG
...
STACK_FOUND=0
for STACK_CANDIDATE in kpmg-lm-logs-forwarder; do
...
if [ "$STACK_FOUND" -eq 0 ]; then
    RESULTS+=("[FAIL] No CloudFormation stack found for KPMGLMLogsForwarder")
    FAIL=$((FAIL + 1))
fi
